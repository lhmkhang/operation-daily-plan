...
const morganStream = {
  write: (message) => {
    const statusCode = parseInt(message.split(' ')[message.split(' ').length - 5], 10);
    if (statusCode < 400) {
...
const serverConfiguration = (app) => {
  ...
  app.use(morgan(":remote-addr :remote-user :method :url HTTP/:http-version :status :res[content-length] - :response-time ms", { stream: morganStream }));
