...
import withAuth from "../helpers/WithAuthen";
    ...
    const spin = () => {
        ...
        setTimeout(async () => {
            ...
            try {
                ...
                console.error("Lỗi khi kiểm tra phần thưởng:", error.name);
                if (error.name === "AxiosError" && error.response.status === 403) {
                    setModalContent(
                        `Phiên đăng nhập đã hết, nhấn F5 để trở về trang đăng nhập`
                    );
                    setModalOpen(true);
                }
            }
        ...
        <div className="main-container">
            <div className="wheel-container">
                ...
                <button
                    ...
                    disabled={isSpinning || totalTurn === 0}
                    title={
                        totalTurn === 0 ? "Bạn không còn lượt quay nào" : "Bắt đầu quay"
                    }
                >
                    Start
                </button>
